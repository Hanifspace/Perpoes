...
<body class="bg-slate-50 text-slate-800">
    ...
    <div class="flex items-center gap-6">
      ...
      <nav class="hidden md:flex items-center gap-6 text-sm text-slate-600">
        ...
        <a href="{{ route('user.pinjam.index') }}"
           class="menu-link {{ request()->routeIs('user.pinjam.*') ? 'active' : '' }}">
          Pinjam Buku
        </a>
        <a href="{{ route('user.favorit.index') }}"
           class="menu-link {{ request()->routeIs('user.favorit.*') ? 'active' : '' }}">
          Favorit
        </a>
  ...
  <div class="md:hidden border-b border-slate-200 bg-white/70 backdrop-blur">
    <div class="px-4 py-3 flex items-center gap-4 text-sm text-slate-600 overflow-x-auto">
      <a href="{{ route('user.dashboard') }}"
         class="menu-link whitespace-nowrap {{ request()->routeIs('user.dashboard') ? 'active' : '' }}">
        Dashboard
      </a>
      <a href="{{ route('user.pinjam.index') }}"
         class="menu-link whitespace-nowrap {{ request()->routeIs('user.pinjam.*') ? 'active' : '' }}">
        Pinjam Buku
      </a>
      <a href="{{ route('user.favorit.index') }}"
         class="menu-link whitespace-nowrap {{ request()->routeIs('user.favorit.*') ? 'active' : '' }}">
        Favorit
      </a>
      <a href="{{ route('user.riwayat.index') }}"
         class="menu-link whitespace-nowrap {{ request()->routeIs('user.riwayat.*') ? 'active' : '' }}">
        Riwayat
      </a>
    </div>
  </div>
