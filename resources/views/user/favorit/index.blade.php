  ...
  <div class="max-w-7xl mx-auto px-6 py-8">
    ...
    <div class="rounded-2xl bg-gradient-to-br from-blue-700 to-blue-900 px-8 py-8 mb-8 flex flex-col items-center text-center gap-5">
      <div>
        ...
        <h1 class="font-display text-3xl text-white leading-tight">Buku Favorit</h1>
        <p class="text-blue-200 text-sm mt-0.5">Buku-buku yang sudah kamu simpan sebagai favorit.</p>
      </div>

      {{-- Search Bar --}}
      <form method="GET" action="{{ route('user.favorit.index') }}" class="w-full max-w-md">
        <div class="relative">
    ...
    <div class="grid gap-5 grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6">
      @forelse ($bukus as $buku)
        ...
        <article class="group flex flex-col overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm hover:shadow-md transition-shadow duration-200">
          ...
          <div class="relative w-full card-cover">
            ...
            <form action="{{ route('user.favorit.destroy', $buku->id) }}" method="POST" class="absolute right-2.5 top-2.5">
              @csrf
              @method('DELETE')
              <button
                type="submit"
      ...
      @empty
        <div class="col-span-full rounded-2xl border border-dashed border-slate-200 bg-white p-12 text-center">
          <i class="fas fa-heart text-3xl text-slate-200 mb-3 block"></i>
          <p class="text-sm text-slate-400">
            {{ request('search') ? 'Buku tidak ditemukan.' : 'Belum ada buku favorit.' }}
          </p>
        </div>
      @endforelse
    </div>
